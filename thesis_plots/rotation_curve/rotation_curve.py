...
class RotationCurve:
    def __init__(self):
        ...
        data = {}
        fns = ['fit', 'pts', 'pts+', 'pts-', 'halo', 'gas', 'stellar_disk', 'luminous+',
               'luminous-']
        for fn in fns:
        ...
        data['x'] = xp
        data['baryons'] = np.sqrt(
            np.interp(xp, data['gas_x'], data['gas']) ** 2 + np.interp(xp, data['stellar_disk_x'],
                                                                       data['stellar_disk']) ** 2)
        data['all'] = np.sqrt(
    ...
    def plot_rotation_curve(self):
        data = self.data
        plt.errorbar(data['pts_x'], data['pts'], data['dpts'], marker='.', capsize=5, ls='None',
                     label='Measured')
